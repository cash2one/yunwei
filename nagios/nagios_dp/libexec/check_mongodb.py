...
def main(argv):
    ...
    p.add_option('-A', '--action', action='store', type='choice', dest='action', default='connect', help='The action you want to take',
                 choices=['connect', 'connections', 'replication_lag', 'replset_state', 'memory', 'lock', 'flushing', 'last_flush_time',
                          ...
                          'write_data_files','journaled','opcounters','current_lock','replica_primary','page_faults','asserts','slavelag'])
    ...
    if action == "connections":
        ...
        return check_opcounters(con,host, warning, critical,perf_data,port)
	...
	return check_slave_lag(con, warning, critical,perf_data)
...
def get_opcounters(data,opcounters_name,host,port):
    ...
    return maintain_delta(new_vals, host,opcounters_name,port)

def check_opcounters(con, host, warning, critical,perf_data,port):
...
def check_current_lock(con, host, warning, critical,perf_data,port):
    ...
    err,delta=maintain_delta([totalTime,lockTime],host,"locktime",port)
    if err==0:
...
def check_page_faults(con, host, warning, critical,perf_data,port):
    ...
    err,delta=maintain_delta([page_faults],host,"page_faults",port)
    if err==0:
...
def check_asserts(con, host, warning, critical,perf_data,port):
    ...
    err,delta=maintain_delta([regular,warning_asserts,msg,user,rollovers],host,"asserts",port)
...
def maintain_delta(new_vals,host,action,port):
    file_name=build_file_name(host,action,port)
...
def check_slave_lag(con, warning, critical, perf_data):
    warning = warning or 180
    critical = critical or 300
    try:
        #set_read_preference(con.admin)
	
        # Get server status
        status = con.admin.command(pymongo.son_manipulator.SON([('serverStatus', 1),('repl',2)]))
        if "sources" in status["repl"]:
                slaveDelays = status["repl"]["sources"][0]["lagSeconds"]
        else:
                print "CRITICAL - No slave defined. Is slave service running?"
		return 2
        message = "slavelag is %.2f seconds" % slaveDelays
        message +=performance_data(perf_data,[(slaveDelays,"slavelag",warning, critical)])
        return check_levels(slaveDelays,warning,critical,message)
