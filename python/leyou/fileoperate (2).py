...
def compress(indir,outdir,compressfile,logfile=None,errlog=None):
	'''压缩函数，
	目前支持最合适的两种压缩方式，bz2压缩率最少，消耗CPU最多，lzo消耗最少，压缩率稍高,gz一般'''
	#初始化日志实例
#	log = phplog(logfile)
#	errlog = phplog(errlog,verbose=False)
#	syslog.openlog(os.path.basename(sys.argv[0]))
	available = ("bz2","lzo","gz")
	suffix = compressfile.rsplit(".",1)[-1]
	if not suffix in available:
		message ='"fail","指定的压缩文件%s不是bz2或者lzo或者gz"' % compressfile
...
#			syslog.syslog(syslog.LOG_ERR,message)
	...
	if suffix == "bz2":
		...
		cmd="cd %s;tar -czf %s/%s %s" % (dir,outdir,compressfile,filename)
	else:
...
#	if not outdir:outdir=os.path.dirname(sys.argv[0])
	if not outdir:outdir=os.path.dirname(file)
	available = ("bz2","lzo","gz")
...
#		syslog.syslog(syslog.LOG_ERR,message)
	...
	if suffix == "bz2":
		...
		cmd = 'content=(`tar --show-stored-names -xvzf %s -C %s`);[ "$?" -eq 0 ] && echo ${content[0]} ||exit 1' % (file,outdir)
		if check:cmd = 'content=(`tar --show-stored-names -tzf %s`);[ "$?" -eq 0 ] && echo ${content[0]} ||exit 1' % file
	else:
...
def chmod(path,mode,R=False):
	if R:
		for dirpath,dirs,files in os.walk(path,topdown=True):
			...
			for file in files:
				os.chmod(os.path.join(dirpath,file),mode)
	else:
		os.chmod(path,mode)	
...
def mkdir(path,mode=755,owner='root',group='root'):
	...
	if run.returncode != 0:
		raise IOError("创建文件夹%s错误,错误代码%s,错误信息:%s" % (path,run.returncode,stderr))
