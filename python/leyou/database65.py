...
class mysql ():
	...
	def backup(self,database,outdir,tables=None,delete=None):
		...
		parameter = {"host":self.host,"port":self.port,"username":self.shell_username,"password":self.shell_password,"database":database,"outdir":outdir}
                cmd = "/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -B %(database)s -d --skip-triggers > %(outdir)s/structure.sql &&\
		/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -R -t -n -d %(database)s --add-drop-trigger > %(outdir)s/routine.sql &&\
		/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s %(database)s --skip-triggers -T %(outdir)s" % parameter
		if tables:
			...
			if not tbs:raise ValueError("实例%s,数据库%s,没有找到匹配正则%s的表" % (self.port,database,tables))
			parameter["tbs"] = " ".join(tbs)
			cmd = """
			/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -B %(database)s -t -d --skip-triggers > %(outdir)s/structure.sql && \
			/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -B %(database)s -d --tables %(tbs)s --skip-triggers >> %(outdir)s/structure.sql &&\
			/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -R -t -n -d -B %(database)s --add-drop-trigger >%(outdir)s/routine.sql &&\
			/usr/local/mysql/bin/mysqldump -h %(host)s -P %(port)d %(username)s %(password)s -B %(database)s --skip-triggers --tables %(tbs)s -T %(outdir)s""" % parameter
                #清空目录下的文件，保证备份干净
		rm(outdir+"/*",rf=True)
		starttime=time.time()
		result = subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
		...
		if delete:
			try:
				...
				if not delete_tables:raise RuntimeError("没有找到实例%s,数据库%s符合条件%s的表" % (self.port,parameter["database"],delete))
	...
	def restore(self,indir=None,database=None,start_datetime=None,datetime=None,delete=None):
		...
		if indir:
			...
			cmd = """structure=`find %(indir)s -name "*structure.sql"`;routine=`find %(indir)s -name "*routine.sql"`;
				 ...
				 old_database=`gawk -F':' 'NR == 3 && /Database:/{gsub(/ /,"",$NF);print $NF}' $structure`
			         ...
			         /usr/local/mysql/bin/mysql -h %(host)s -P %(port)d %(username)s %(password)s -D %(database)s <$routine""" % parameter
			starttime=time.time()
			result = subprocess.Popen(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
                	stdout,stderr = result.communicate()
			processTime = time.time() - starttime
		...
		if datetime:
			...
			if not indir:
				backup_datetime = start_datetime
				try:
					bin_log_sql = self.binlog_to_sql(datetime,parameter["database"],backup_datetime)
                                        message = '"success","导出时间范围%s-%s的二进制日志到%s"' % (backup_datetime,datetime,bin_log_sql)
                                        syslog.syslog(message)
